...
laid out as a fixed table and organized into **collections**
...
- **Pan & zoom** — drag and scroll the whole table on a clean canvas.
...
- **`src/components/GlifMap.tsx`** — the whole surface. The glifs are a fixed-pixel
  CSS grid, so the 3px gap stays constant at any zoom. Pan/zoom gestures accumulate
  and apply to the grid **once per animation frame**. Static canvas, no backend.
